            ...
            <tbody class="list">
                @foreach ($data as $index => $user)
                    <tr class="row-clickable" data-id="{{ \Crypt::encrypt($user->id) }}">
                        ...
                        <td class="Action">
                            @can('edit producer')
                                <button type="button" onclick="editProducerUser('{{ $user->id }}')" class="btn btn-success btn-sm">
                                    <i class="bi bi-pencil"></i>
                                </button>
                            @endcan
                            @can('delete producer')
                                <button type="button" onclick="deleteProducerUser('{{ $user->id }}')" class="btn btn-danger btn-sm">
