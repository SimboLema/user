                            ...
                            <div class="card-header card-header-stretch">
                                <div class="card-title">
                                    <h3 class="m-0 text-gray-800">Endorsements <span style="color: #003153;">(Sum Insured:
                                            TZS {{ number_format($quotation->sum_insured) }})</span></h3>
                                </div>
                                ...
                                <div class="modal-dialog modal-lg modal-dialog-centered">
                                    <div class="modal-content">
                                        <form id="createEndorsementForm"
                                            action="{{ route('quotation.makeEndorsement', $quotation->id) }}"
                                            method="POST">
                                            @csrf
                                ...
                                <div id="kt_endorsements_tab_pane_1" class="card-body p-0 tab-pane fade show active"
                                    ...
                                    <div class="table-responsive">
                                        <table class="table align-middle table-row-bordered table-row-solid gy-4 gs-9">
                                            ...
                                            <tbody>
                                                @foreach ($quotationEndorsements as $quotationEndorsement)
                                                    <tr class="fs-6 fw-semibold border-bottom-2">
                                                        <td>
                                                            {{ Carbon\Carbon::parse($quotationEndorsement->created_at)->format('d M Y, h:i:A') }}
                                                        </td>
                                                        <td>
                                                            {{ $quotationEndorsement->request_id }}
                                                        </td>
                                                        <td>
                                                            {{ $quotationEndorsement->endorsement_premium_earned != null ? number_format($quotationEndorsement->endorsement_premium_earned) : 'none' }}
                                                        </td>
                                                        <td>{{ $quotationEndorsement->payment->paymentMode->name ?? 'N/A' }}
                                                        </td>
                                                        {{-- <td>{{ \Illuminate\Support\Str::limit($quotationEndorsement->description, 20, '...') }}
                                                        </td> --}}
                                                        <td>{{ $quotationEndorsement->endorsementType->name }}
                                                        </td>

                                                        <td class="text-center">
                                                            @if ($quotationEndorsement->status === 'pending')
                                                                <a href="{{ route('quotation.sendTira.Endorsement', $quotationEndorsement->id) }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function calculatePremium() {
                ...
                const startDate = new Date('{{ $quotation->cover_note_start_date }}');
                const endDate = new Date('{{ $quotation->cover_note_end_date }}');
