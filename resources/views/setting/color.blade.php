            ...
            <form class="row g-3 needs-validation" novalidate id="form" onsubmit="save(event)" enctype="multipart/form-data">
                ...
                <div class="modal-body overflow-auto"  style="max-height: 400px;">
                    <div class="row">
                        ...
                        <!-- Code -->
                        <div class="col-md-6 form-group">
                            <label for="name" class="form-label">Value<span style="color:red">*</span></label>
                            <input type="color" class="form-control" id="value" name="value"  required>
                        </div>
...
<script>
    ...
    function editColor(id) { // Updated function name
        document.getElementById('form').reset();
        $("#hidden_id").val("");

        jQuery.ajax({
            type: "GET",
            url: "/color/edit/" + id, // Updated URL
            dataType: 'json',
            success: function (data) {
    ...
    function deleteColor(id) { // Updated function name
        var conf = confirm("Are you sure you want to delete this color?");
        if (!conf) {
            return;
        }
        ...
        jQuery.ajax({
            type: "GET",
            url: "/color/delete/" + id, // Updated URL
            dataType: 'json',
            success: function (data) {
                if (data.status == 200) {
                    showFlashMessage("success", data.message);
