            ...
            <div class="modal-body overflow-auto" style="max-height: 400px;">
                <div class="row g-6">
                    ...
                    <div class="col-md-6 form-group">
                        ...
                        <input type="text" class="form-control" id="name" name="name" placeholder="Enter Collection Center Name" required>
                    </div>
                    ...
                    <div class="col-md-6 form-group">
                        <label for="district_color" class="form-label">Color Code</label>
                        <input type="color" class="form-control form-control-color" id="district_color" name="district_color">
                    </div>

                    {{-- <div class="col-md-12 form-group">
                        <label class="form-label">Pick Location on Map <span style="color:red">*</span></label>
...
<script>
    ...
    function clear_input() {
        ...
        $("#hidden_id").val("")
        $("#submitBtn").html("Save")
        $("#modalTitle").html("New Collection Center");
    }
    ...
    function editCollectionCenter(id) {
        ...
        jQuery.ajax({
            type: "GET",
            url: "/collection_center/edit/" + id,
            dataType: 'json',
            success: function (data) {
                let row = data.data;
                var district = row.district || {};
                var ward = row.ward || {};
                var region = (district && district.region) || {};
                var country = (region && region.country) || {};

                $("#hidden_id").val(data.id)
                $("#name").val(row.name);
                $("#district_id").val(row.district_id);
                $("#ward_id").val(row.ward_id);
                $("#district_color").val(row.district_color);
                ...
                $("#modalTitle").html("Update Collection Center");
    ...
    function deleteCollectionCenter(id) {
        if (!confirm("Are you sure you want to delete this collection center?")) return;

        jQuery.ajax({
            type: "GET",
            url: "/collection_center/delete/" + id,
            dataType: 'json',
            success: function (data) {
                if (data.status == 200) {
                    showFlashMessage("success", data.message);
