            ...
            <div class="tab-content pt-4" id="wasteTabsContent">
                ...
                <div class="tab-pane fade show active" id="overview" role="tabpanel">
                    <div class="row g-4">
                        @php
                            $details = [
                                ['label' => 'Waste Picker', 'value' => $waste->wastePicker->user->name ?? '-', 'icon' => 'bi-building'],
                            ];

                            if ($waste->collectionCenter) {
                                $details[] = ['label' => 'Collection Center', 'value' => $waste->collectionCenter->name ?? '-', 'icon' => 'bi-pin-map'];
                                $details[] = ['label' => 'Collected By', 'value' => $waste->collectionUser->user->name ?? '-', 'icon' => 'bi-person'];
                            }
                ...
                <div class="tab-pane fade" id="items" role="tabpanel">
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-sm">
                            ...
                            <tbody>
                                    <tr>
                                        <td>{{ $waste->wasteType->name ?? '-' }}</td>
                                        <td>{{ $waste->quantity }}</td>
                                        <td>{{ $waste->unit->name ?? '-' }}</td>
                                        <td>{{ $waste->color->name ?? '-' }}</td>
                                    </tr>
